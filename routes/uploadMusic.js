...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // const results = await cloudupAPI.upload({
...
router.put("/update/:id", async (req, res) => {
  ...
  try {
    const mp3 = await UploadMusic.findById(req.params.id);
...
router.get("/daata", async (req, res) => {
  try {
    const mp3s = await UploadMusic.find({}).sort({ createdAt: -1 });
...
router.get("/mp3/:title", async (req, res) => {
  try {
    const mp3 = await UploadMusic.findOne({
...
router.get("/mp3/songs/:artist/:title", async (req, res) => {
  try {
    ...
    const mp3 = UploadMusic.findOne({ artist: artist, title: title });
    if (!mp3) {
      return res.status(404).json({ error: "Music not found" });
    }
...
router.get("/mp3s/:artist/:title", async (req, res) => {
  try {
    const mp3 = await UploadMusic.find({
      $and: [
...
router.get("/", async (req, res) => {
  try {
    const mp3s = await UploadMusic.find(
      {},
...
router.get("/get/:id", async (req, res) => {
  ...
  const mp3 = await UploadMusic.findById(id);
  if (!mp3) {
